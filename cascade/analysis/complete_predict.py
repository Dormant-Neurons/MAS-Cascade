#!/usr/bin/env python3
"""
FJ Complete (Fully-Connected) Topology Prediction: Fixed-parameter and Incremental.

Complete topology:
  - Every agent is connected to every other agent
  - Each agent i has learned neighbor weights W[i, j] for all j ≠ i
    parameterized via per-agent softmax over (N-1) logits (sum to 1)

Model (per agent, vectorized over K answer options):
  b_i(t+1) = γ_i * s_i + (1-γ_i) * [α_i * b_i(t) + (1-α_i) * Σ_{j≠i} W_{ij} * b_j(t)]
...
Usage:
  python complete_predict.py <belief_logs_dir>             # single directory
  python complete_predict.py <root_dir> --recursive        # find all belief files under root_dir
"""
...
def unpack_theta(theta: np.ndarray, N: int):
    ...
    theta layout:  [gamma × N | alpha × N | W_logits × N*(N-1)]
    ...
    for i in range(N):
        ...
        weights = stable_softmax(theta[offset: offset + (N - 1)])
...
def train_loss(theta: np.ndarray, S_train: np.ndarray, b0: np.ndarray, N: int) -> float:
...
def initial_fit(S_train: np.ndarray, b0: np.ndarray, N: int):
    ...
    theta0 = np.concatenate([np.full(N, 0.2), np.full(N, 0.5), np.zeros(N * (N - 1))])
    bounds = [(0.0, 1.0)] * (2 * N) + [(None, None)] * (N * (N - 1))
    return minimize(
        train_loss, theta0, args=(S_train, b0, N),
...
def run_fixed(S_full: np.ndarray, rounds: np.ndarray, N: int,
    ...
    train_pred  = np.stack(train_preds)
    ...
    summary = {
        ...
        "num_rounds_total": T_total, "topology": "complete",
    ...
    return summary, params, perround
...
def main():
    ...
    print(f"Processing {len(files)} files across {len(groups)} run(s)  [complete topology]")
    ...
    for belief_logs_dir, group_files in sorted(groups.items()):
        ...
        fixed_summaries, fixed_params, fixed_perround = [], [], []
        ...
        for path in sorted(group_files):
            ...
            try:
                ...
                fs, fp, fperr = run_fixed(S_full, rounds, N, b0, question_id)
        ...
        print()
        save(fixed_summaries, "complete_fixed_summary.csv")
        save(fixed_params,    "complete_fixed_params.csv")
        save(fixed_perround,  "complete_fixed_perround.csv")
        save(inc_summaries,   "complete_incremental_summary.csv")
        save(inc_perround,    "complete_incremental_perround.csv")
        save(inc_params,      "complete_incremental_params.csv")
        print()
